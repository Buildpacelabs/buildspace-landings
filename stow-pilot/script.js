/* StowPilot — interactions
   IntersectionObserver reveals, count-ups, self-drawing walk-distance line,
   driver bars, split ring, mini-bars. All motion-safe. */
(function () {
  'use strict';
  ...
  function countUp(el, done) {
    ...
    function format(v) {
      ...
      return decimals === 0 ? Number(n).toLocaleString('en-US') : n;
    }
    if (reduceMotion) {
    ...
    function frame(t) {
      ...
      else { el.textContent = format(target); if (done) done(); }
    }
    requestAnimationFrame(frame);
  }

  /* ---- Self-drawing walk-distance line ---- */
  function drawWalk(scope) {
    var line = scope.querySelector('.walk-line');
    var area = scope.querySelector('.walk-area');
    var dot = scope.querySelector('.walk-dot');
    if (!line) return;
  ...
  var activators = new IntersectionObserver(function (entries) {
    entries.forEach(function (e) {
      ...
      if (el.classList.contains('walk-chart')) drawWalk(el);
      activators.unobserve(el);
    });
  }, { threshold: 0.35 });

  document.querySelectorAll('[data-count], .driver-bar, .ring-fill, .mini-bar, .walk-chart')
    .forEach(function (el) { activators.observe(el); });

  /* ---- Hero walk chart draws on load (above the fold) ---- */
  var heroChart = document.querySelector('.hero .walk-chart');
  if (heroChart) {
    window.addEventListener('load', function () { setTimeout(function () { drawWalk(heroChart); }, 300); });
  }
