/* DockQueue — interactions
   IntersectionObserver reveals, count-ups, why-bars, dwell/detention ring,
   yard gauge and mini-bars. All motion-safe. */
(function () {
  'use strict';
  ...
  /* ---- One-shot activators (count, bars, rings) ---- */
  var activators = new IntersectionObserver(function (entries) {
    entries.forEach(function (e) {
      if (!e.isIntersecting) return;
      var el = e.target;
      if (el.hasAttribute('data-count')) countUp(el);
      if (el.classList.contains('why-bar')) {
        var pct = el.getAttribute('data-pct');
        var i = el.querySelector('i');
        if (i) i.style.width = pct + '%';
      }
      if (el.classList.contains('ring-fill') || el.classList.contains('yg-fill')) {
  ...
  document.querySelectorAll('[data-count], .why-bar, .ring-fill, .yg-fill, .mini-bar')
    .forEach(function (el) { activators.observe(el); });

  /* ---- Hero yard gauge draws on load (above the fold) ---- */
  var heroGauge = document.querySelector('.hero .yg-fill');
  if (heroGauge) {
    window.addEventListener('load', function () {
      setTimeout(function () {
        var dash = heroGauge.getAttribute('data-dash');
        heroGauge.style.strokeDasharray = dash + ' 999';
      }, 300);
    });
  }
