/* VitalLoop — interactions
   IntersectionObserver reveals, count-ups, self-drawing ECG line,
   signal bars, adherence ring. All motion-safe. */
(function () {
  'use strict';
  ...
  /* ---- Self-drawing line (ECG) ---- */
  function drawLine(scope) {
    var line = scope.querySelector('.ecg-line');
    var area = scope.querySelector('.ecg-area');
    var dot = scope.querySelector('.ecg-dot');
    if (!line) return;
    ...
    if (area) { area.style.transition = 'opacity 1.4s ease 0.6s'; area.style.opacity = '1'; }
    if (dot) { dot.style.transition = 'opacity 0.5s ease 1.6s'; dot.style.opacity = '1'; }
  }
  ...
  var activators = new IntersectionObserver(function (entries) {
    entries.forEach(function (e) {
      ...
      if (el.classList.contains('signal-bar')) {
        var i = el.querySelector('i');
        if (i) i.style.width = el.getAttribute('data-pct') + '%';
      }
      if (el.classList.contains('ring-fill') || el.classList.contains('rf')) {
      ...
      if (el.classList.contains('ecg-chart')) drawLine(el);
      activators.unobserve(el);
    });
  }, { threshold: 0.35 });

  document.querySelectorAll('[data-count], .signal-bar, .ring-fill, .rf, .mini-bar, .ecg-chart')
    .forEach(function (el) { activators.observe(el); });

  /* ---- Hero ECG draws on load (above the fold) ---- */
  var heroChart = document.querySelector('.hero .ecg-chart');
  if (heroChart) {
    window.addEventListener('load', function () { setTimeout(function () { drawLine(heroChart); }, 300); });
  }
